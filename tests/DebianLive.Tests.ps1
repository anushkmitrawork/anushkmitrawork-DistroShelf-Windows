# Explicit opt-in live integration test for Debian Track and Profile.
...
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\tests\DebianLive.Tests.ps1

$ErrorActionPreference='Stop'
if($env:DISTROSHELF_RUN_LIVE_TESTS -ne '1'){
    Write-Host 'SKIP  Debian live integration test is opt-in. Set DISTROSHELF_RUN_LIVE_TESTS=1 to run it.'
    exit 0
}
...
$provider=Get-DistroShelfProvider -Distro 'Debian'
$trackDef=Get-DistroShelfTrackDefinition -Distro 'Debian'
if(Test-Path -LiteralPath $trackDef.Root){Fail 'Refusing live run because committed Debian Track 0 already exists; remove it only through a deliberate test reset.'}
Pass 'Debian Track 0 is absent before live build'

$trackBuild=Invoke-DistroShelfTrackBuilder -Distro 'Debian' -OnProgress {param($p,$m)Write-Host ("[TRACK {0,3}%] {1}" -f [int]$p,$m)}
if(-not $trackBuild.Success){Fail "Debian Track build failed: $($trackBuild.Error)`nTroubleshoot: $($trackBuild.TroubleshootPath)"}
...
if([string]$trackBuild.FinalHash -notmatch '^[0-9a-f]{64}$'){Fail 'Debian Track final hash is not SHA-256.'}
Pass 'Debian Track completed with every provider stage verified'

$trackCommit=Commit-DistroShelfTrackTransaction -BuildResult $trackBuild
if(-not $trackCommit.Success){Fail 'Debian Track commit failed.'}
if(-not(Test-DistroShelfTrackIntegrity -Distro 'Debian')){Fail 'Committed Debian Track failed final integrity verification.'}
Pass 'Debian Track 0 committed and integrity-verified'

$trackMeta=Get-DistroShelfTrackManifest -Distro 'Debian'
if([string]$trackMeta.FinalHash -ne [string]$trackCommit.FinalHash){Fail 'Committed Track final hash differs from build result.'}
Pass 'Committed Debian Track hash matches verified build result'

$reservation=Reserve-DistroShelfProfileNumber -Distro 'Debian'
$candidate=[pscustomobject]@{Id=$reservation.Id;Name=$reservation.Name;Distro='Debian';WslName="DistroShelf-$($reservation.Name)";PackageManager='apt';Status='Candidate';Terminal='GNOME Console'}
try {
    $profile=Invoke-DistroShelfProfileBuild -Distro 'Debian' -Terminal 'GNOME Console' -TrackRoot $trackDef.Root -TrackHash ([string]$trackMeta.FinalHash) -Candidate $candidate -OnProgress {param($p,$m)Write-Host ("[PROFILE {0,3}%] {1}" -f [int]$p,$m)}
    if(-not $profile.Success){Fail "Debian Profile build failed: $($profile.Error)`nTroubleshoot: $($profile.TroubleshootPath)"}
    if([string]$profile.ProfileHash -notmatch '^[0-9a-f]{64}$'){Fail 'Debian Profile hash is not SHA-256.'}
    ...
    Pass 'Debian Profile passed full acceptance and produced its single accepted export'

    $profileCommit=Commit-DistroShelfProfileTransaction -BuildResult $profile -Reservation $reservation -Terminal 'GNOME Console'
    if(-not $profileCommit.Success){Fail 'Debian Profile commit failed.'}
    $reservation=$null
    ...
    Pass "Debian Profile $($candidate.Name) committed from the exact accepted export"
}
finally {
    if($reservation){try{Release-DistroShelfProfileReservation -Reservation $reservation}catch{}}
}

Write-Host "`nDebian live Track + Profile integration test passed."
