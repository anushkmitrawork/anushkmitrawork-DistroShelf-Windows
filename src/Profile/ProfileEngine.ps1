...
function Invoke-DistroShelfProfileBuild {
    ...
    try {
        if(-not(Test-DistroShelfHashRecord -Path (Join-Path (Join-Path $TrackRoot 'metadata') 'track.hash.json') -Root $TrackRoot -Stage 'track' -ExcludeRelativePath @('metadata/track.hash.json','metadata/track.json'))){throw "Verified Track hash is missing or invalid for '$Distro'."}
        ...
        return [pscustomobject]@{Success=$true;Transaction=$tx;Candidate=$candidate;ProfileRoot=$profileRoot;ProfileHash=$profileHash;Acceptance=$acceptance}
    } catch {
