...
function New-DistroShelfUbuntuProvider {
    ...
    $names=@('gnome-terminal','kitty','alacritty','foot','konsole');$t=New-DistroShelfPackageStage 'terminals' 'apt' $names (@($names|%{New-StageTest $_ "command -v $_"})) 'terminals';$t.Kind='terminal-set'
    ...
    [pscustomobject][ordered]@{SchemaVersion=1;Distro='Ubuntu';Track='Ubuntu0';PackageManager='apt';Rootfs=@{Name='Ubuntu';Architecture='amd64'};Stages=@($root,$p,$d,$f,$fl,$ds,$t);TrackFinalTests=@(New-StageTest 'podman-functional' 'podman run --rm quay.io/podman/hello true');ProfileFinalTests=@(New-StageTest 'profile-os' 'test -s /etc/os-release')}
}
