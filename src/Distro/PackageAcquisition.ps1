...
# These commands are for Track construction only. Profile installation must consume local artifacts.
...
function New-DistroShelfPackageStage {
    ...
    switch($Manager){
        'apt' {
            ...
            $profile="apt-get -y --no-download install /track/$Id/packages/*.deb"
            $export='apt-cache'
        }
        'dnf' {
            ...
            $profile="dnf -y --disablerepo='*' install /track/$Id/packages/*.rpm"
            $export='rpm-cache'
        }
        'pacman' {
            ...
            $profile="pacman -U --noconfirm /track/$Id/packages/*.pkg.tar.*"
            $export='pacman-cache'
        }
        'zypper' {
            ...
            $profile="zypper --non-interactive install /track/$Id/packages/*.rpm"
            $export='rpm-cache'
        }
