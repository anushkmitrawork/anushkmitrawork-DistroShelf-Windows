...
function New-StageContract {
    param([string]$Id,[string[]]$Depends,[string]$PackageManager,[string[]]$TrackAcquire,[string[]]$TrackInstall,[object[]]$TrackTests,[string[]]$ProfileInstall,[object[]]$ProfileTests,[string]$ExportType,[string]$ExportValue,[string]$Kind='dependency',[string]$ParallelGroup,[string]$ResourceLock,[ValidateSet('SharedBuilder','IsolatedBuilder')][string]$ExecutionModel='SharedBuilder')
    [pscustomobject][ordered]@{Id=$Id;Depends=@($Depends);Kind=$Kind;ParallelGroup=$ParallelGroup;PackageManager=$PackageManager;ResourceLock=$ResourceLock;ExecutionModel=$ExecutionModel;Track=[pscustomobject][ordered]@{Acquire=@($TrackAcquire);Install=@($TrackInstall);Tests=@($TrackTests);ExportType=$ExportType;ExportValue=$ExportValue};Profile=[pscustomobject][ordered]@{Install=@($ProfileInstall);Tests=@($ProfileTests)}}
}
function New-DistroShelfRootfsStage {param([string]$Manager)[pscustomobject][ordered]@{Id='rootfs';Depends=@();Kind='rootfs';ParallelGroup='bootstrap';PackageManager=$Manager;ResourceLock='wsl-rootfs';ExecutionModel='SharedBuilder';Track=[pscustomobject][ordered]@{Acquire=@();Install=@();Tests=@(New-StageTest 'os-release' 'test -s /etc/os-release');ExportType='none';ExportValue=''};Profile=[pscustomobject][ordered]@{Install=@();Tests=@(New-StageTest 'os-release' 'test -s /etc/os-release')}}
...
function Test-DistroShelfProviderContract {
    ...
    if(!$stages.Count){$errors+="Provider '$($Provider.Distro)' has no stages."}
    $ids=@{}
    foreach($stage in $stages){
        $id=[string]$stage.Id
        if(!$id){$errors+='Provider contains a stage without an Id.';continue}
        if($ids.ContainsKey($id)){$errors+="Duplicate stage '$id'."}else{$ids[$id]=$true}
        if(!$stage.Track -or !$stage.Profile){$errors+="Stage '$id' is missing Track/Profile contracts.";continue}
        ...
        if([string]$stage.ExecutionModel -notin @('SharedBuilder','IsolatedBuilder')){$errors+="Stage '$id' has invalid ExecutionModel."}
    }
    foreach($stage in $stages){foreach($d in @($stage.Depends)){if(!$ids.ContainsKey([string]$d)){$errors+="Stage '$($stage.Id)' depends on unknown stage '$d'."}}}
