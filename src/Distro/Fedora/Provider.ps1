...
function New-DistroShelfFedoraProvider {
    ...
    [pscustomobject][ordered]@{SchemaVersion=2;Distro='Fedora';Track='Fedora0';PackageManager='dnf';Rootfs=@{Name='Fedora';Architecture='amd64'};Stages=@($root,$p,$d,$f,$fl,$ds)+$terminalStages;TrackFinalTests=@(New-StageTest 'podman-functional' 'podman run --rm quay.io/podman/hello true');ProfileFinalTests=@(New-StageTest 'profile-os' 'test -s /etc/os-release')}
}
