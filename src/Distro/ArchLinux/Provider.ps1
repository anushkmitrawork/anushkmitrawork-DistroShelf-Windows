...
function New-DistroShelfArchLinuxProvider {
    ...
    $names=@('kitty','alacritty','foot','konsole');$t=New-DistroShelfPackageStage 'terminals' 'pacman' $names (@($names|%{New-StageTest $_ "command -v $_"})) 'terminals';$t.Kind='terminal-set'
    ...
    [pscustomobject][ordered]@{SchemaVersion=1;Distro='Arch Linux';Track='ArchLinux0';PackageManager='pacman';Rootfs=@{Name='Arch Linux';Architecture='amd64'};Stages=@($root,$p,$d,$f,$fl,$ds,$t);TrackFinalTests=@(New-StageTest 'podman-functional' 'podman run --rm quay.io/podman/hello true');ProfileFinalTests=@(New-StageTest 'profile-os' 'test -s /etc/os-release')}
}
