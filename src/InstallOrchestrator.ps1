...
function Invoke-DistroShelfInstall {
    ...
    try {
        ...
        if(Test-Path -LiteralPath $finalTrack -PathType Container){
            ...
            $oldOverride=$env:DISTROSHELF_TRACK_ROOT_OVERRIDE;$env:DISTROSHELF_TRACK_ROOT_OVERRIDE=$env:LOCALAPPDATA
            try{$existingComplete=Test-DistroShelfTrackComplete -Distro $Distro}finally{$env:DISTROSHELF_TRACK_ROOT_OVERRIDE=$oldOverride}
            if(-not $existingComplete){$quarantine=Join-Path (Join-Path $env:LOCALAPPDATA 'DistroShelf\Troubleshoot') ("PreviousTrack-{0}-{1}" -f $Distro,(Get-Date -Format 'yyyyMMdd-HHmmss'));New-Item -ItemType Directory -Path $quarantine -Force|Out-Null;Move-Item -LiteralPath $finalTrack -Destination (Join-Path $quarantine (Split-Path $finalTrack -Leaf)) -Force}
        }
        $parent=Split-Path $finalTrack -Parent;New-Item -ItemType Directory -Path $parent -Force|Out-Null
        Move-Item -LiteralPath (Join-Path $trackRoot (Split-Path $finalTrack -Leaf)) -Destination $finalTrack -Force
        ...
        try { if($profile){$wslPath=Join-Path $wslStorage $profile.WslName;if(Test-Path -LiteralPath $wslPath -PathType Container){Copy-Item -LiteralPath $wslPath -Destination (Join-Path $trouble 'Wsl') -Recurse -Force}} } catch {}
        try { if($wslImported -and $profile){& wsl.exe --unregister $profile.WslName 2>$null|Out-Null} } catch {}
        ...
        $record=Join-Path $trouble 'failure.txt';@("Distro: $Distro","Profile candidate: $($profile.Name)","Wsl name: $($profile.WslName)","Error: $message","Created: $([DateTime]::UtcNow.ToString('o'))")|Set-Content -LiteralPath $record -Encoding UTF8
        Report-Progress 100 "Installation failed. Attempt preserved in Troubleshoot."
