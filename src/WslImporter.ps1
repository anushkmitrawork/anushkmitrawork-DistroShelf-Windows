...
function Invoke-DistroShelfWslImport {
    ...
    $verify=@(& wsl.exe --list --verbose 2>$null)-join "`n";if($verify -notmatch [regex]::Escape($Profile.WslName)){throw "WSL import completed without a verifiable '$($Profile.WslName)' registration."}
