    ...
    try {
        while($queue.Count -or $active.Count){
            ...
            foreach($job in $completed){
                try{
                    $value=$job.PowerShell.EndInvoke($job.AsyncResult)
                    $results+=[pscustomobject]@{Stage=$job.Stage;Success=$true;Result=@($value)}
                }catch{
                    $results+=[pscustomobject]@{Stage=$job.Stage;Success=$false;Error=$_.Exception.Message}
                    if(-not $firstError){$firstError=$_.Exception}
                }finally{
