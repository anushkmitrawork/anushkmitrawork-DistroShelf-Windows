...
function Write-DistroShelfHashRecord {
    ...
    $record=[ordered]@{SchemaVersion=2;Stage=$Stage;Algorithm='SHA256';Hash=$Hash;Tests=$TestResult;CreatedAt=[DateTime]::UtcNow.ToString('o')};$tmp="$Path.tmp";$record|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $tmp -Encoding UTF8;Move-Item -LiteralPath $tmp -Destination $Path -Force;return [pscustomobject]$record
}
function Test-DistroShelfHashRecord {
    ...
    if(-not(Test-Path -LiteralPath $Path -PathType Leaf)){return $false};try{$r=Get-Content -LiteralPath $Path -Raw|ConvertFrom-Json;if([string]$r.Stage-ne$Stage-or[string]::IsNullOrWhiteSpace([string]$r.Hash)){return $false};if([string]$r.Algorithm-ne'SHA256'){return $false};return (Get-DistroShelfTreeHash -Root $Root -ExcludeRelativePath $ExcludeRelativePath)-eq([string]$r.Hash).ToLowerInvariant()}catch{return $false}
}