...
function Write-DistroShelfHashRecord {
    ...
    $tmp = "$Path.tmp"
    $record | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $tmp -Encoding UTF8
    Move-Item -LiteralPath $tmp -Destination $Path -Force
    return [pscustomobject]$record
}

function Test-DistroShelfHashRecord {
    param([Parameter(Mandatory)][string]$Path,[Parameter(Mandatory)][string]$Root,[Parameter(Mandatory)][string]$Stage)
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { return $false }
    try {
        $r = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
        if ([string]$r.Stage -ne $Stage -or [string]::IsNullOrWhiteSpace([string]$r.Hash)) { return $false }
        return (Get-DistroShelfTreeHash -Root $Root) -eq ([string]$r.Hash).ToLowerInvariant()
    } catch { return $false }
}
